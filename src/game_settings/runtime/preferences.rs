//! Less frequently used Sunrise settings exposed in application Preferences.
use super::{
    fields::{Field, Kind},
    optional_value,
};
...
pub(super) const FIELDS: &[Field] = &[
    ...
    Field {
        ...
        help: "Configuration route served by the external server; 1–127 printable ASCII characters.",
    },
    Field {
...
pub(crate) fn draw(ui: &mut egui::Ui, document: &mut Value, json_account: bool) -> bool {
    if !super::available(document) {
        return false;
    }
    ui.strong("Sunrise Configuration");
    ui.label("These edits belong to the open settings file. Save, then fully restart Destiny 2 to apply. Omitted fields retain Sunrise's defaults.");
