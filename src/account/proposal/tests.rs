...
fn automatic_cleanup_respects_the_selected_account_backend_build() {
    ...
    .unwrap();
    assert!(crate::account::validate_authored_cleanup_backend(&settings).is_err());
    for version in [6, 8, 17] {
        std::fs::write(&settings, format!("{{\"version\":{version}}}")).unwrap();
        assert!(crate::account::validate_authored_cleanup_backend(&settings).is_ok());
        assert!(
            crate::account::validate_authored_cleanup_backend(
