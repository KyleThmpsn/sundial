use super::*;
use crate::account::AuthoredSlotChange;
use serde_json::json;
...
fn slot_replacement_selects_json_or_sqlite_at_runtime_without_touching_inactive_json() {
    use crate::account::{preview_replacement, replace_authored_account_source};
    ...
    let json = preview_replacement(&settings, &BTreeSet::new(), &[], &[], Some(&slots)).unwrap();
    ...
    let sqlite = preview_replacement(&settings, &BTreeSet::new(), &[], &[], Some(&slots)).unwrap();
    ...
    replace_authored_account_source(&database, &sqlite.original_bytes, &sqlite.cleaned_bytes)
        .unwrap();
    assert_eq!(
        crate::account::read_authored_account_source(&database).unwrap(),
