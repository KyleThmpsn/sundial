...
use crate::persistence::{
    json_account::{equipment, inventory},
    progression,
};
...
pub fn preview_replacement(
    ...
    // This is a Sunrise settings source. The authoring bridge selects Dawn explicitly from the
    // installed runtime, never from a database left beside another runtime's settings.
    ...
    super::validate_authored_cleanup_backend(settings_path)?;
...
fn clean(
    ...
    let cleared = progression::mutations::remove_authored_collection_state(&mut document, unlocks)?;
    let mut rewards = 0;
