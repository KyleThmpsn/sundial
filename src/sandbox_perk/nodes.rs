...
pub const EFFECTS: [NodeKind; 55] = [
    ...
    NodeKind {
        ...
        occurrences: 1011,
        name: "Create Entity",
        summary: "Attaches a referenced entity, with configurable cleanup when the action ends.",
        evidence: "1089400 creates the graph on EC0AA0's selected object and can initialize a named parameter from action state. 108B550 handles removal: a nonempty cleanup key suppresses the automatic retirement branch, and an optional named four-lane parameter is written to the entity. The key alone does not select the entity's gameplay behavior.",
        support: Support::Authorable,
    },
    NodeKind {
        kind: 2,
    ...
    NodeKind {
        ...
        struct_size: 16,
        occurrences: 2,
        name: "Radar Overrides",
        summary: "Overrides radar settings and restores their previous values when the effect ends.",
        evidence: "108D4B0 calls D9A310 for radar component slots 1, 2 and 3. D98420 and D8F2D0 propagate these settings to the player HUD. Negative values leave a setting unchanged. Slot 3 is detection range, independently identified by Long March and Radar Booster. The first two settings remain unresolved.",
        support: Support::Authorable,
    },
    NodeKind {
        kind: 19,
    ...
    NodeKind {
        ...
        occurrences: 2,
        name: "Enhanced Radar",
        summary: "Adds or removes a contribution to radar detail, reversing it when the effect ends.",
        evidence: "D8EBF0 adjusts radar component counter +90 and clamps it at zero. D98420 calls D8F2D0, which adds one to the HUD radar detail mode while the counter is positive and clamps the mode to 0 through 2. Upgraded Sensor Pack activates this effect while crouching. It is separate from radar visibility while aiming.",
        support: Support::Authorable,
    },
    NodeKind {
        kind: 21,
    ...
    NodeKind {
        ...
        struct_size: 184,
        occurrences: 43,
        name: "Incoming Damage Multiplier",
        summary: "Multiplies matching incoming damage while the effect is active, optionally within a maximum source distance.",
        evidence: "1088E80 evaluates damage filter +80, then compares squared source-to-recipient distance with +C squared when +C is nonnegative. Selector +8 equal to FF uses multiplier +4, otherwise it reads a native stat. CD3020 registers the descriptor with the health component and CDDDD0 removes it. CD3570 consumes its result as an incoming-damage multiplier. Native stat identities remain unresolved.",
        support: Support::Authorable,
    },
    NodeKind {
        kind: 34,
