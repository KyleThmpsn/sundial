...
use super::{Progression, integer, invalid};
...
impl Progression {
    pub(super) fn apply_artifact_seed(
        ...
        for row in rows {
            ...
            {
                return Err("Artifact state changed before the edit could be applied".to_owned());
            }
...
fn artifact_flags(value: &Value) -> Result<BTreeMap<i32, i32>, String> {
    let rows = value
        .as_array()
