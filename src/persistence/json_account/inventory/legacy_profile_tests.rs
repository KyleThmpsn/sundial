        ...
        for &gear_class in gear_classes {
            for &masterworked in masterwork_filters {
                let candidate = (
                    ...
                    gear_class.map_or(0, gear_class_mask),
...
fn dismantle_policy_key(snapshot: &DismantleRewardSnapshot) -> (u32, u8, u8, u8) {
    (
        ...
        snapshot.gear_class.map_or(0, gear_class_mask),
        snapshot
            .masterworked
...
fn rarity_mask_of(rarities: &[DismantleRarity]) -> u8 {
    rarities
        .iter()
        .fold(0, |mask, rarity| mask | rarity_bit(*rarity))
}
...
fn write_dismantle_policy(
    ...
    if filtered && !rarities.is_empty() {
        let value = if rarities.len() == 1 {
            Value::String(rarity_token(rarities[0]).into())
        } else {
            Value::Array(
                rarities
                    .iter()
...
const fn gear_class_mask(gear_class: DismantleGearClass) -> u8 {
    match gear_class {
        DismantleGearClass::Weapon => 1,
        DismantleGearClass::Armor => 2,
        DismantleGearClass::Both => 3,
    }
}
