...
pub(crate) const KNOWN_ITEM_MEMBERS: &[&str] = &[
    "instance_soid",
    "definition_hash",
    "level",
    "quantity",
...
pub(in crate::persistence::json_account::inventory) fn set_inventory_flag(
    flags: Option<u8>,
    flag: u8,
    enabled: bool,
) -> Option<u8> {
...
pub(in crate::persistence::json_account::inventory) const NO_DEFINITION_HASH: u32 =
...
impl SchemaMode {
    ...
    pub(in crate::persistence::json_account::inventory) const fn enforces_profile_item_capacity(
        self,
    ) -> bool {
        !matches!(self, Self::Future(_))
    }

    pub(in crate::persistence::json_account::inventory) const fn enforces_character_inventory_capacity(
        self,
    ) -> bool {
        !matches!(self, Self::Future(_))
    }
...
pub(in crate::persistence::json_account::inventory) fn require_readable_schema(
    document: &Value,
...
#[cfg(test)]
pub(in crate::persistence::json_account::inventory) fn require_profile_mutation(
    document: &Value,
) -> InventoryResult<SchemaMode> {
...
pub(in crate::persistence::json_account::inventory) fn require_inventory_mutation(
    document: &Value,
) -> InventoryResult<SchemaMode> {
    let mode = require_readable_schema(document)?;
...
pub(in crate::persistence::json_account::inventory) fn read_only_schema_error(
    mode: SchemaMode,
    section: &str,
) -> InventoryError {
    match mode {
