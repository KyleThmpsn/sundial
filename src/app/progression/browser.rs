...
pub(super) fn draw(ui: &mut egui::Ui, document: &Value, catalog: &Catalog, state: &mut UiState) {
    let browse_values = state.unlock_table == super::state::UnlockTable::ValueDefinitions;
    let definitions = if browse_values {
    ...
    let rows = definitions
        .iter()
        .enumerate()
        .filter(|(index, definition)| matches(&query, *index, definition))
        .collect::<Vec<_>>();
    ui.label(format!(
        "{} / {} Definitions",
        rows.len(),
    ...
    egui::ScrollArea::both()
        .id_salt("definition_browser_rows")
        .max_height(ui.available_height().max(TABLE_ROW_STRIDE * 3.0))
        .auto_shrink([false, false])
        .show_rows(ui, TABLE_CELL_HEIGHT, rows.len(), |ui, range| {
            egui::Grid::new("definition_browser_grid")
                .striped(true)
                .show(ui, |ui| {
                    for offset in range {
                        let (index, definition) = rows[offset];
                        if ui.small_button(format!("Inspect #{index}")).clicked() {
                            state.metadata_inspector.open(if browse_values {
                                MetadataSelection::ValueDefinition(index)
                            } else {
                                MetadataSelection::FlagDefinition(index)
                            });
                        }
                        ...
                        let name = definition_name(definition)
                            ...
                            .map_or_else(
                                || format!("References: {}", override_meaning(definition)),
                                str::to_owned,
                            );
                        table_cell(ui, 220.0, &name).on_hover_ui(|ui| {
                            crate::ui_help::tooltip_title(ui, &name);
                            ui.label(definition_metadata_tooltip(definition));
                        });
                        ui.label(format!(
                            "Bank {} · Slot {}",
                            definition.bank(),
                        ...
                        let saved = snapshot.as_ref().map_or_else(
                            || "Unavailable".into(),
                            |snapshot| {
                                if browse_values {
                        ...
                        let evaluated = snapshot.as_ref().and_then(|snapshot| {
                            if browse_values {
                                snapshot
                                ...
                                snapshot
                                    ...
                                    .map(|value| if value { "Set" } else { "Clear" }.into())
                            }
                        });
                        ui.label(format!(
                            "Evaluated: {}",
                        ...
                        ui.label(format!(
                            ...
                            definition.runtime_writers.len()
                        ));
                        ui.end_row();
                    }
                });
